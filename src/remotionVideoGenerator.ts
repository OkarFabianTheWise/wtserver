...
import { IllustrationVideo } from './remotion/IllustrationVideo.js';
import { generateAnimationScript } from './animationScriptGenerator.js';
import type { AnimationScript } from './types.js';
...
export async function generateIllustrationVideoWithRemotion(
    script: string,
    audioBuffer: Buffer,
    outputPath?: string
): Promise<Buffer> {
    ...
    try {
        ...
        const animationScript: AnimationScript = await generateAnimationScript(script);
...
function combineVideoWithAudio(videoPath: string, audioPath: string, outputPath: string): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        console.log(`🔊 Video file exists: ${fs.statSync(videoPath).size} bytes`);
        console.log(`🔊 Audio file exists: ${fs.statSync(audioPath).size} bytes`);
        ...
        ffmpeg()
            ...
            .on('end', () => {
                ...
                if (fs.existsSync(outputPath)) {
                    console.log(`✅ Output file created: ${fs.statSync(outputPath).size} bytes`);
                }
                resolve();
            })
            .on('error', (err) => {
